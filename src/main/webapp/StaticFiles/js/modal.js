...
let warehousingGrid3 = document.querySelector("#warehousingGrid3");
...
const getListData2 = (cusCode) => {
    let xhr = new XMLHttpRequest();
    xhr.open('GET',
        '/receiv/deliverylistForReceive?customerCode='+cusCode,
        true);
    xhr.setRequestHeader('Accept', 'application/json');
    xhr.send();
    xhr.onreadystatechange = () => {
...
let stockinsertGridOptions;
const setStockModal = () => {
    stockinsertGridOptions = gridOptions;
    stockinsertGridOptions.columnDefs = [
        ...
        {headerName: '임시견적재고량', field: 'tempQuoAmount',}
    ];
    stockinsertGridOptions.getRowNodeId = (data) => {
        return data.orderNo;
    };
    new agGrid.Grid(warehousingGrid3, stockinsertGridOptions);
}


const getStockModal = () => {
    let xhr = new XMLHttpRequest();
    xhr.open('GET', '/stock/sto/list',
        true);
    xhr.setRequestHeader('Accept', 'application/json');
    xhr.send();
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
            stockinsertGridOptions.api.setRowData([]);
            let txt = xhr.responseText;
            txt = JSON.parse(txt);
            console.log(txt);
            if (txt.errorCode < 0) {
            ...
            stockinsertGridOptions.api.updateRowData({add: txt.gridRowJson});
        }
    }
}
...
const setListModal = () => {
    ...
    listGridOptions.columnDefs = [
		...
		let selectedRows=contractGridOptions.api.getSelectedNodes();     //수주옵션이나와야함
		console.log("여기???"+selectedRows);
		if(selectedRows!=undefined){
			selectedRows.map(selectedData =>{
				selectedData.data.contractType=to.detailCode;
                console.log("타입???"+selectedData.data.contractType);
				selectedData.data.contractTypeName=to.detailCodeName;
			})
		}
            let ss=contractGridOptions.api.redrawRows(selectedRows);
            console.log("이게뭐지??"+ss);
...
const setListModal2 = () => {
    ...
    let delListGridOptions = gridOptions;
    delListGridOptions.columnDefs = [
        {headerName: '납품일련번호', field: 'deliveryNo', width: 80, cellStyle: {'textAlign': 'center'},},
        {headerName: '거래처번호', field: 'customerCode', width: 80, cellStyle: {'textAlign': 'center'},},
        {headerName: '처리자', field: 'personCodeInCharge', width: 80, cellStyle: {'textAlign': 'center'}}];
    delListGridOptions.onRowDoubleClicked = (event) => {
        console.log(event);
        console.log('event.data.detailCodeName:'+event.data);
        to = event.data;
        let closeList = document.querySelectorAll('.close');
        console.log("이게뭔데?"+closeList);
        for (let value of closeList) {
            console.log("value???"+value);
            value.click();
        }

        let selectedRows=delListGridOptions.api.getSelectedNodes();
        if(selectedRows!=undefined){
            selectedRows.map(selectedData =>{
                selectedData.data.deliveryNo=to.deliveryNo;
                //더블클릭하면 선택한값이 그리드에 입력되게함
            })
        }
        delListGridOptions.api.redrawRows(selectedRows);
    };
    console.log(listGrid);
    if(listGrid!=null){
        listGrid.innerHTML="";
        new agGrid.Grid(listGrid, delListGridOptions);
        delListGridOptions.api.setRowData(jsonData.gridRowJson);
    }
};
...
const setWarehousingModal = () => {
    ...
    warehousingGridOptions.columnDefs = [
        ...
        {headerName: '검사상태', field: 'inspectionStatus', cellStyle: {'textAlign': 'center'},
            cellRenderer: function (params) {
            if (params.value == "Y") {
                return params.value =  "🟢" ;
            }
